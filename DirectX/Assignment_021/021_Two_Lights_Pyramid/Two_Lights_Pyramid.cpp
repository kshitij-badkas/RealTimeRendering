...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName,
		TEXT("D3D11: Assignment_021_Two_Lights_Pyramid : By Kshitij Badkas"),
...
HRESULT Initialize(void)
{
	...
	const char* vertexShaderSourceCode =
		...
		"output.phong_ads_light = ambient[0] + diffuse[0] + specular[0] + ambient[1] + diffuse[1] + specular[1];" \
		"}" \
		"}" \
		"else" \
		"{" \
	...
	//NORMALS
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(pyramidNormals); //glBufferData() 2nd param
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;  //glBindBuffer() , vbo
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC; //DYNAMIC_DRAW - glBufferData() 4th or last paramater
	...
	memcpy(mappedSubresource.pData, pyramidNormals, sizeof(pyramidNormals)); //push data from cpu buffer to gpu buffer
	...
	light[0].lightPosition[0] = -2.0f;
	light[0].lightPosition[1] = 0.0f;
	light[0].lightPosition[2] = 0.0f;
...
////////////////////////////////////////////////////////////
	...
	light[1].lightDiffuse[1] = 0.0f;
	light[1].lightDiffuse[2] = 1.0f; //BLUE
	light[1].lightDiffuse[3] = 1.0f;

	light[1].lightSpecular[0] = 0.0f;
	light[1].lightSpecular[1] = 0.0f;
	light[1].lightSpecular[2] = 1.0f;
	light[1].lightSpecular[3] = 1.0f;

	light[1].lightPosition[0] = 2.0f;
	light[1].lightPosition[1] = 0.0f;
	light[1].lightPosition[2] = 0.0f;
...
void Display(void)
{
	...
	if (gbLights == true)
	{
		...
		for (int i = 0; i < 2; i++)
		{
			constantBuffer.la[i] = XMVectorSet(light[i].lightAmbient[0], light[i].lightAmbient[1], light[i].lightAmbient[2], light[i].lightAmbient[3]);
			constantBuffer.ld[i] = XMVectorSet(light[i].lightDiffuse[0], light[i].lightDiffuse[1], light[i].lightDiffuse[2], light[i].lightDiffuse[3]);
			constantBuffer.ls[i] = XMVectorSet(light[i].lightSpecular[0], light[i].lightSpecular[1], light[i].lightSpecular[2], light[i].lightSpecular[3]);
			constantBuffer.lightPosition[i] = XMVectorSet(light[i].lightPosition[0], light[i].lightPosition[1], light[i].lightPosition[2], light[i].lightPosition[3]);
		}
