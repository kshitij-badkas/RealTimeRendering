...
HRESULT Initialize(void)
{
	...
	//COLOR
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(pyramidColors); //glBufferData() 2nd param
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;  //glBindBuffer() , vbo
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC; //DYNAMIC_DRAW - glBufferData() 4th or last paramater
	...
	//COLOR
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cubeColors); //glBufferData() 2nd param
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;  //glBindBuffer() , vbo
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC; //DYNAMIC_DRAW - glBufferData() 4th or last paramater
	...
	memcpy(mappedSubresource.pData, pyramidColors, sizeof(pyramidColors)); //push data from cpu buffer to gpu buffer
	...
	memcpy(mappedSubresource.pData, cubeColors, sizeof(cubeColors)); //push data from cpu buffer to gpu buffer
...
void Display(void)
{
	...
	rotationMatrix = XMMatrixRotationX(angleSquare) * XMMatrixRotationY(angleSquare) * XMMatrixRotationZ(angleSquare);
	...
	//ANIMATION LOGIC
	angleTriangle += 0.00015f;
	if (angleTriangle >= 360.0f)
	{
		angleTriangle = 0.0f;
	}

	angleSquare -= 0.00015f;
