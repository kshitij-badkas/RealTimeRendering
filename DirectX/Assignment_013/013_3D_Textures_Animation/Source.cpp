...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName,
		TEXT("D3D11: Assignment_013_3D_Shapes_Textures : By Kshitij Badkas"),
...
HRESULT Initialize(void)
{
	...
	//5. decalre pixel shader source code
			...
			// Texture2D - similar to texture() of GLSL
			// SamplerState - similar to uniform sampler2D of GLSL 
	...
	inputElementDesc[1].SemanticIndex = 0; //if you are going to send multiple geometry to same semantic name 
										//then they are separated by indices. 
										//Hence it is 0 for 1 geometry.
	inputElementDesc[1].Format = DXGI_FORMAT_R32G32_FLOAT; //glVertexAttribPointer() 2nd paramater
	inputElementDesc[1].AlignedByteOffset = D3D11_APPEND_ALIGNED_ELEMENT; //glPixelStorei(GL_UNPACK_ALIGNMENT)
	...
	//TEXCOORD
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(pyramidTexcoord); //glBufferData() 2nd param
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;  //glBindBuffer() , vbo
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC; //DYNAMIC_DRAW - glBufferData() 4th or last paramater

	// because we are not using Static Draw we will not use 2nd param
	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_Texcoord_Pyramid);

	if (FAILED(hr))
	{
		fopen_s(&gpFile, gpszLogFileName, "a+");
		fprintf_s(gpFile, "gpID3D11Device::CreateBuffer() Failed for Vertex Buffer - PYRAMID .\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gpszLogFileName, "a+");
		fprintf_s(gpFile, "gpID3D11Device::CreateBuffer() Succeeded for Vertex Buffer - PYRAMID.\n");
		fclose(gpFile);
	}
	...
	//POSITION
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cubeVertices); //glBufferData() 2nd param
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;  //glBindBuffer() , vbo
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC; //DYNAMIC_DRAW - glBufferData() 4th or last paramater
	...
	//TEXCOORD
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cubeTexcoord); //glBufferData() 2nd param
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;  //glBindBuffer() , vbo
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC; //DYNAMIC_DRAW - glBufferData() 4th or last paramater

	// because we are not using Static Draw we will not use 2nd param
	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_Texcoord_Cube);

	if (FAILED(hr))
	{
		fopen_s(&gpFile, gpszLogFileName, "a+");
		fprintf_s(gpFile, "gpID3D11Device::CreateBuffer() Failed for Vertex Buffer - CUBE .\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gpszLogFileName, "a+");
		fprintf_s(gpFile, "gpID3D11Device::CreateBuffer() Succeeded for Vertex Buffer - CUBE.\n");
		fclose(gpFile);
	}

	//11. b Push the data in this buffer  -PYRAMID
	D3D11_MAPPED_SUBRESOURCE mappedSubresource;
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Position_Pyramid, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	...
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Texcoord_Pyramid, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, pyramidTexcoord, sizeof(pyramidTexcoord)); //push data from cpu buffer to gpu buffer
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Texcoord_Pyramid, 0);

	//11. b Push the data in this buffer  -CUBE
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Position_Cube, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, cubeVertices, sizeof(cubeVertices)); //push data from cpu buffer to gpu buffer
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Position_Cube, 0);

	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Texcoord_Cube, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, cubeTexcoord, sizeof(cubeTexcoord)); //push data from cpu buffer to gpu buffer
	...
	//TEXTURE
	//PYRAMID
	hr = loadD3DTexture(L"Stone.bmp", &gpID3D11ShaderResourceView_Texture_Pyramid); //L - wchar_t LITERAL
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gpszLogFileName, "a+");
	...
	{
		fopen_s(&gpFile, gpszLogFileName, "a+");
		fprintf_s(gpFile, "loadD3DTexture() Succeeded - PYRAMID.\n");
		fclose(gpFile);
	}

	//CUBE
	hr = loadD3DTexture(L"Vijay_Kundali.bmp", &gpID3D11ShaderResourceView_Texture_Cube); //L - wchar_t LITERAL
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gpszLogFileName, "a+");
	...
	{
		fopen_s(&gpFile, gpszLogFileName, "a+");
		fprintf_s(gpFile, "gpID3D11Device::CreateSamplerState() Failed - CUBE.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gpszLogFileName, "a+");
		fprintf_s(gpFile, "gpID3D11Device::CreateSamplerState() Succeeded - CUBE.\n");
		fclose(gpFile);
	}
...
void Display(void)
{
	...
	rotationMatrix = XMMatrixRotationX(angleCube) * XMMatrixRotationY(angleCube) * XMMatrixRotationZ(angleCube);
	XMMATRIX scaleMatrix = XMMatrixScaling(0.85f, 0.85f, 0.85f);
