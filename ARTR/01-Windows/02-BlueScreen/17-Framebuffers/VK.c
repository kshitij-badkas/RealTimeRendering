...
VkResult Initialize(void)
{
	...
	if (vkresult != VK_SUCCESS)
	{
		fprintf(gpFile, "\nInitialize() -> createFramebuffers() failed : %d.\n", vkresult);
		return (vkresult);
	}
	else
	{
		fprintf(gpFile, "\nInitialize() -> createFramebuffers() Succeeded: %d.\n", vkresult);
	}
...
void UnInitialize(void)
{
	...
	for (uint32_t i = 0; i < swapchainImageCount; i++)
	{
		...
		fprintf(gpFile, "\nUnInitialize() -> vkDestroyFramebuffer() IS DONE.");
	}
	if (vkframebuffer_array)
	{
		free(vkframebuffer_array);
		vkframebuffer_array = NULL;
		fprintf(gpFile, "\nUnInitialize() -> vkframebuffer_array IS DONE.");
	}
...
VkResult createFramebuffers(void)
{
	...
	// declare arr of vkImageView
	VkImageView vkimageView_attachment_array[1];

	memset((void *)vkimageView_attachment_array,
				 0,
				 sizeof(VkImageView) * _ARRAYSIZE(vkimageView_attachment_array));

	// declare and init VkFramebufferCreateInfo
	...
	vkframebufferCreateInfo.layers = 1; // vulkan provides 256 layers guaranteed
	// desktop quality GPU vendors support 2048 layers
	...
	for (uint32_t i = 0; i < swapchainImageCount; i++)
	{
		...
		if (vkresult != VK_SUCCESS)
		{
			fprintf(gpFile, "\ncreateFramebuffers -> vkCreateBuffer failed  for i = %d ::: %d", i, vkresult);
			return (vkresult);
		}
		else
		{
			fprintf(gpFile, "\ncreateFramebuffers -> vkCreateBuffer succeeded for i = %d ::: %d", i, vkresult);
		}
	} // for loop end
